"""
dashboard.py — SRX Platform | Sober & Adaptive Institutional Dashboard

FILE LOCATION: ~/Desktop/srx-platform/frontend/dashboard.py
...
# =============================================================================
# DYNAMIC THEME ENGINE — stored in session_state, swapped via f-string CSS
# =============================================================================

if "theme" not in st.session_state:
    st.session_state.theme = "Dark"

# Theme palettes — every color the UI needs, keyed by mode.
THEMES = {
    "Dark": {
        "bg":           "#0f131a",
        "sidebar_bg":   "#131820",
        ...
        "divider":      "#1e293b",
        "method_bg":    "#1b2231",
        "method_border": "#2d3748",
        "method_term":  "#cbd5e1",
        "method_desc":  "#94a3b8",
        "method_header": "#94a3b8",
...
# Risk colors — consistent across both themes (amber for elevated, red for critical).
R = {
    "safe":      "#4a7c59",
    "elevated":  "#c49032",
    "critical":  "#b83232",
    ...
    "fill_dark": "rgba(132, 148, 167, 0.06)",
    "fill_light": "rgba(90, 106, 122, 0.06)",
    "sub1":      "#5c7fa0",
    "sub2":      "#8a7e6a",
    "sub3":      "#7a6888",
    "sub4":      "#5c8a72",
...
<style>
    ...
    h1 {{ color: {t['heading_1']} !important; font-weight: 600 !important; letter-spacing: -0.01em; }}
    h2 {{ color: {t['text_primary']} !important; font-weight: 500 !important; }}
    h3 {{ color: {t['heading_3']} !important; font-weight: 500 !important; font-size: 1.05rem !important; }}
    ...
    .method-box {{
        background-color: {t['method_bg']};
        border: 1px solid {t['method_border']};
        border-radius: 6px;
        padding: 20px 24px;
        margin: 8px 0 20px 0;
        line-height: 1.65;
    ...
    .method-box h4 {{
        color: {t['method_header']};
        font-size: 0.75rem;
        text-transform: uppercase;
        letter-spacing: 0.08em;
        margin: 0 0 14px 0;
        padding-bottom: 10px;
        border-bottom: 1px solid {t['method_border']};
    }}
    .method-box .def-term {{
        color: {t['method_term']};
        font-weight: 600;
    ...
    .gate-banner h1 {{ margin: 0 !important; font-size: 1.4rem !important; }}
    .gate-banner p {{ margin: 5px 0 0 0; font-size: 0.9rem; }}
...
def _chart_layout(height=420, **kwargs):
    """Adaptive chart layout — switches template and colors based on active theme."""
    base = dict(
        template=t["chart_template"],
        height=height,
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        font=dict(color=t["chart_font"], size=11),
        margin=dict(l=48, r=24, t=32, b=48),
        legend=dict(orientation="h", y=-0.14, font=dict(size=10, color=t["chart_font"])),
        xaxis=dict(gridcolor=t["chart_grid"], gridwidth=1),
        yaxis=dict(gridcolor=t["chart_grid"], gridwidth=1),
    )
    base.update(kwargs)
    return base

fill_color = R["fill_dark"] if st.session_state.theme == "Dark" else R["fill_light"]
...
def call_backend(endpoint: str, params: dict = None) -> dict:
    try:
        response = requests.get(f"{BACKEND_URL}{endpoint}", params=params, timeout=120)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.ConnectionError:
        return {"error": "Cannot connect to the backend server. Ensure uvicorn is running on port 8000."}
    except requests.exceptions.Timeout:
        return {"error": "Request timed out. Try fewer tickers or a shorter period."}
    except Exception as e:
        return {"error": f"Backend request failed: {e}"}


def show_error(result: dict) -> bool:
...
# =============================================================================
# SIDEBAR — Theme toggle + all inputs
# =============================================================================
...
with st.sidebar:
    ...
    liquidity_adj = st.slider("Liquidity Adjustment", 0.5, 2.0, 1.0, step=0.05, help="1.0 = standard. Higher values reflect illiquid portfolios.")
...
# ---- Parse inputs ----
tickers_str = ",".join([t_sym.strip().upper() for t_sym in ticker_input.split(",") if t_sym.strip()])
...
if page == "Overview":
    ...
    with col1:
        with st.spinner("Loading GSRI..."):
            gsri_data = call_backend("/gsri", {"period": period})
        if not show_error(gsri_data):
            st.metric("GSRI", f"{gsri_data['current_gsri']:.1f}", gsri_data["risk_level"],
                       help="Global Systemic Risk Index — aggregate systemic risk benchmark across major asset classes.")
    with col2:
    ...
    with col3:
        with st.spinner("Loading PRI..."):
            pri_params = {"tickers": tickers_str, "period": period}
            if weights_str:
                pri_params["weights"] = weights_str
            pri_data = call_backend("/pri", pri_params)
    ...
    if isinstance(gsri_data, dict) and "gsri_history" in gsri_data:
        ...
        if not history_df.empty:
            ...
            fig.add_trace(go.Scatter(x=history_df["date"], y=history_df["gsri"], mode="lines", name="GSRI",
                                      line=dict(color=R["line"], width=2), fill="tozeroy", fillcolor=fill_color))
            ...
            fig.add_hline(y=30, line_dash="dot", line_color=R["safe"], annotation_text="Low", annotation_font_size=10, annotation_font_color=t["text_muted"])
            fig.add_hline(y=55, line_dash="dot", line_color=R["elevated"], annotation_text="Elevated", annotation_font_size=10, annotation_font_color=t["text_muted"])
            fig.add_hline(y=75, line_dash="dot", line_color=R["critical"], annotation_text="Critical", annotation_font_size=10, annotation_font_color=t["text_muted"])
...
# =============================================================================
    ...
    st.markdown('<p class="narrative">Composite risk assessment of the user-defined portfolio across volatility, correlation, concentration, and tail risk dimensions.</p>', unsafe_allow_html=True)

    if st.button("Calculate PRI", type="primary"):
        with st.spinner("Computing PRI..."):
            params = {"tickers": tickers_str, "period": period}
            if weights_str:
                params["weights"] = weights_str
            result = call_backend("/pri", params)
        if not show_error(result):
            ...
            with col_left:
                ...
                fig = go.Figure(data=go.Scatterpolar(r=vals + [vals[0]], theta=cats + [cats[0]], fill="toself",
                    fillcolor="rgba(132, 148, 167, 0.10)" if st.session_state.theme == "Dark" else "rgba(90, 106, 122, 0.08)",
                    line=dict(color=R["line"], width=2)))
                fig.update_layout(**_chart_layout(380),
                    polar=dict(radialaxis=dict(visible=True, range=[0, 100], gridcolor=t["chart_grid"], color=t["chart_font"]),
                               angularaxis=dict(color=t["chart_font"]), bgcolor="rgba(0,0,0,0)"))
                st.plotly_chart(fig, use_container_width=True)
            with col_right:
                ...
                if result.get("cumulative_return_pct") is not None:
                    ...
                    st.write(f"PRI End Value: **{result.get('pri_end_value', 'N/A')}**")

    st.markdown("---")
    st.markdown("### Systemic Risk Scores (SRS) — Per Asset")
    st.markdown('<p class="narrative">Individual asset contribution to systemic risk, measured via beta, volatility, and tail behavior.</p>', unsafe_allow_html=True)
    if st.button("Calculate SRS"):
        with st.spinner("Computing SRS..."):
            srs_result = call_backend("/srs", {"tickers": tickers_str, "period": period})
...
# =============================================================================
    ...
    if st.button("Calculate GSRI", type="primary"):
        with st.spinner("Computing GSRI & SRS..."):
            result = call_backend("/gsri", {"period": period, "rolling_window": rolling_window})
        if not show_error(result):
            col_left, col_right = st.columns([1, 2])
            with col_left:
                st.metric("Current GSRI", f"{result['current_gsri']:.1f}", help="Global Systemic Risk Index — aggregate systemic risk benchmark across major asset classes.")
                st.info(result["risk_level"])
                st.metric("Current SRS", f"{result.get('srs_current', 0):.1f}", help="0–100 score. 0–70 = Normal. 70–85 = Elevated. Above 85 = Critical.")
...
# =============================================================================
    ...
    st.markdown('<p class="narrative">Scenario analysis projecting portfolio impact, stressed PRI, and recovery estimates under historical and hypothetical market dislocations.</p>', unsafe_allow_html=True)
    if st.button("Run Stress Test", type="primary"):
        with st.spinner("Running stress test..."):
            params = {"tickers": tickers_str, "scenario": scenario, "period": period}
            if weights_str: params["weights"] = weights_str
            if custom_shock is not None: params["custom_shock_pct"] = custom_shock
            result = call_backend("/stress", params)
        if not show_error(result):
            c1, c2, c3, c4 = st.columns(4)
            ...
            stressed_pri = result.get("stressed_pri")
            if stressed_pri is not None:
                c4.metric("Stressed PRI", f"{stressed_pri:.1f}", result.get("stressed_risk_level", ""))
            st.markdown("---")
            ...
            fig = go.Figure(go.Waterfall(name="Impact", orientation="v", x=impact_df["ticker"], y=impact_df["weighted_impact_pct"],
                ...
                connector=dict(line=dict(color=t["chart_grid"], width=1)),
...
# =============================================================================
    ...
    st.markdown('<p class="narrative">Model-implied crash protection premium based on notional exposure, systemic stress conditions, and portfolio-specific risk characteristics.</p>', unsafe_allow_html=True)
    if st.button("Price Protection", type="primary"):
        with st.spinner("Pricing protection contract..."):
            params = {"tickers": tickers_str, "notional": notional, "protection_level_pct": protection_level, "duration_days": duration, "period": period}
            if weights_str: params["weights"] = weights_str
            result = call_backend("/price", params)
        if not show_error(result):
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Premium", f"${result['premium_dollars']:,.2f}", help="The model-implied crash protection estimate based on notional, systemic stress, and portfolio-specific risk conditions.")
            ...
            with cr:
                ...
                if mb:
                    st.dataframe(pd.DataFrame([
                        ...
                        {"Component": "Combined Multiplier", "Value": f"{mb.get('combined_multiplier', 1):.4f}x"},
                    ]), hide_index=True, use_container_width=True)
                else:
                    st.write(f"GSRI Multiplier: **{result['gsri_multiplier']:.2f}x**")
                    st.write(f"Annual Premium: **${result['annualized_premium_dollars']:,.2f}**")
...
# =============================================================================
    ...
    st.markdown('<p class="narrative">This simulation models the absorption of a major default event through the five-layer SRX capital structure.</p>', unsafe_allow_html=True)
    event_magnitude = st.number_input("Event Magnitude ($)", min_value=0, max_value=10_000_000_000, value=500_000_000, step=50_000_000, format="%d",
        help="Hypothetical loss: e.g. 100000000 ($100M), 500000000 ($500M), 1000000000 ($1B)")
    ...
    if st.button("Run Waterfall Stress Test", type="primary"):
        result = call_backend("/waterfall", {"defaulter_margin": dm, "defaulter_guarantee_fund": ddf,
...
# =============================================================================
    ...
    st.markdown('<p class="narrative">Platform circuit breakers triggered by SRS thresholds — automatically restricting activity to preserve clearinghouse integrity during stress.</p>', unsafe_allow_html=True)
    ...
    result = call_backend("/gating", {"gsri_score": manual_srs / 1.1, "portfolio_pri": manual_pri})
    if not show_error(result):
        ...
        fig.add_trace(go.Indicator(mode="gauge+number", value=manual_srs,
            title={"text": "Systemic Risk Score", "font": {"size": 13, "color": t["text_muted"]}},
            number={"font": {"color": t["text_primary"]}},
            gauge=dict(axis=dict(range=[0, 100], tickcolor=t["gauge_tick"]),
                bar=dict(color=R["neutral"]),
                steps=[dict(range=[0, 70], color=t["gauge_zone_0"]), dict(range=[70, 85], color=t["gauge_zone_1"]),
                       dict(range=[85, 95], color=t["gauge_zone_2"]), dict(range=[95, 100], color=t["gauge_zone_3"])],
                threshold=dict(line=dict(color=t["gauge_needle"], width=2), value=manual_srs))))
